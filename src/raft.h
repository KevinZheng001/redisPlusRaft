 ...
 * applied to the FSM. */
typedef raft_entry_t raft_entry_t;
...
typedef struct
{
    ...
    /** leader当前已经确认提交到状态机FSM的日志索引index，这意味着Follower也
     *  可以安全地将该索引index以前的日志提交 */
    int leader_commit;
...
typedef struct
{
    ...
    /** node成功添加日志时返回ture，即prev_log_index和prev_log_term都比对成功。
     *  否则返回false */
    int success;
...
typedef int (*func_node_has_sufficient_logs_f)   (
    raft_server_t* raft,
    void *user_data,
    raft_node_t* node
    );
 ...
 * @return 0 on success */
typedef int (*func_persist_int_f)   (
    raft_server_t* raft,
    void *user_data,
    int node
    );
 ...
 * @return 0 on success */
typedef int (*func_logentry_event_f)   (
    raft_server_t* raft,
    void *user_data,
    raft_entry_t *entry,
    int entry_idx
 ...
 *  This is obtained using raft_node_get_link.
 *  Examples of what this could be:
 *  - void* pointing to implementor's networking data
 *  - a (IP,Port) tuple
 ...
 *
 * @note The memory pointer (ie. raft_entry_data_t) for each raft_entry_t is
 ...
 * The log_offer callback will be called.
 *
 * @note The memory pointer (ie. raft_entry_data_t) in raft_entry_t is
 *  copied directly. If the memory is temporary you MUST either make the
 *  memory permanent (ie. via malloc) OR re-assign the memory within the
 *  log_offer callback.
 *
 ...
 */
int raft_recv_entry_from_client(raft_server_t* me, raft_entry_t* ety, msg_entry_response_t *r);
...
/**
 * @return this node's user data */
void* raft_node_get_link(raft_node_t* me);

/**
 * Set this node's user data */
void raft_node_set_link(raft_node_t* me, void* user_data);
